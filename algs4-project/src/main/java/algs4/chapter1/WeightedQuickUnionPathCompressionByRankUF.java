 ...
 */
public class WeightedQuickUnionPathCompressionByRankUF {
    private int[] parent; // parent[i]表示第i个元素所指向的父节点
    private int[] rank;  // rank[i]表示以i为根的集合所表示的树的层数
    private int count; // 数据分量个数

    public WeightedQuickUnionPathCompressionByRankUF(int n) {
        count = n;
        parent = new int[n];
        rank = new int[n];
        for (int i = 0; i < n; i++) {
    ...
    public static void main(String[] args) {
        int N = StdIn.readInt();
        WeightedQuickUnionPathCompressionByRankUF uf = new WeightedQuickUnionPathCompressionByRankUF(N);
